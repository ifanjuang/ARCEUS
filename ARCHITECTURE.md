...
- accès utilisateur ;
- knowledge documentaire ;
- RAG simple sur documents.

Ne doit pas :

- définir les agents officiels ;
- porter la mémoire validée ;
- gouverner les actions ;
- remplacer les Markdown de référence.
...
- charger les skills ;
- appeler les outils ;
- gérer sessions, providers, gateways et scheduler ;
- exposer CLI, gateway, batch runner, API server, ACP ou Python library selon usage ;
- gérer les optional skills installées explicitement.

Ne doit pas :

- devenir source de vérité des règles Pantheon ;
- promouvoir seul une mémoire durable ;
- activer une skill community ou optional sans review Pantheon ;
- accéder aux volumes, secrets ou Docker socket sans policy.
...
- définir les domain packages ;
- définir les skills métier ;
- définir les workflows ;
- définir les policies ;
- définir les règles de mémoire ;
- vérifier les skills Hermes disponibles avant création ;
- encadrer l’usage des skills Hermes.
...
Les domaines métier contiennent leurs skills et workflows spécialisés.

---

# 5. Hermes skill strategy
